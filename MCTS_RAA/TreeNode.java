...
public class TreeNode
{
    ...
    private final double REDUNDANCY_PENALTY = 0.5;
    ...
    public int currentNumberEvents = 0;
    public int lastNumberEvents = 0;
    public ACTIONS lastChosenAction = null;
    ...
    public TreeNode expand(StateObservation state) {
        ...
        tn.lastChosenAction = actions[bestAction];
        tn.lastNumberEvents = events;
    ...
    {//List<Types.ACTIONS> simulation = new ArrayList();
        ...
        lastNumberEvents = state.getEventsHistory().size();

        while (!finishRollout(state,thisDepth)) {
            //int action = m_rnd.nextInt(num_actions);
            //Types.ACTIONS act = actions[action];
            
            currentNumberEvents = state.getEventsHistory().size();
    ...
    private void updateActionMap(Types.ACTIONS last_action) {
        ...
        if(last_action != null){
            ...
            if(opposite_actions.size() > 0){
                 double penalty_value = weight * REDUNDANCY_PENALTY;
                decremented_weight = penalty_value/(double)opposite_actions.size();
                weight += ((weight - penalty_value)/(double)(actions.length-opposite_actions.size()));
            }
        }   
...
//                System.out.println(printAction(action)+":(1) "+weight);
                actionHashMap.put(action, weight);
                rc.add(weight, action);
            }
            else{
//                System.out.println(printAction(action)+":(2) "+decremented_weight);
...
//        HashMap<Types.ACTIONS, Integer> map = new HashMap<>();
//        for(ACTIONS action : actions){
//            map.put(action, 0);
//        }
//        for(int i = 0; i < 1000; i++){
//            Types.ACTIONS next = rc.next();
//            map.put(next, map.get(next)+1);
//        }
//        System.out.println("Results:");
//        for(ACTIONS action : actions){
//            System.out.println(printAction(action)+": "+map.get(action));
//            map.put(action, 0);
//        }
    }
    ...
    */
    private Types.ACTIONS nextRolloutAction() {
        ...
        if(lastNumberEvents == currentNumberEvents){
            updateActionMap(lastChosenAction);
            
            ACTIONS chosen_action = (Types.ACTIONS) rc.next();
            //System.out.println("Chosen Action: "+printAction(chosen_action));      
            Agent.possiblyRedundantActions.add(new Pair<>(lastChosenAction, chosen_action));
            //System.out.println("Saving pair: "+printAction(lastChosenAction)+", "+printAction(chosen_action));
            lastChosenAction = chosen_action;
...
//        System.out.println("Last: "+lastNumberEvents+", Current: "+currentNumberEvents);
